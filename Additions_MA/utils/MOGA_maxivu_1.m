...
% ---------------------------------------------------------
function [X,fval,exitflag,output,population,score] = MOGA_maxivu_1(X0)%, besti)
%global rrr
...
if controlgcp==1
   ...
   c.saveProfile
   c.NumThreads = 2;
   parpool('aurora R2022a',120) %48 %96
% % % %    c.NumThreads = 1;
% % % %    parpool('local',12) %48
   pp = gcp; 
   usepar = true;
...
options = optimoptions(options,'ConstraintTolerance',ConstTol);
