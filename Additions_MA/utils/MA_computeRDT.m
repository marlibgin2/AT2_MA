function [RDT b2L b3L rdt]=MA_computeRDT(ring, index, varargin)
...
%
% MA - re-calculate terms by hand
%

%
% 1st order chromatic terms
%
rdt.h11001 = 1/4*cumsum((b2L - 2*b3L.*etax).*betax); 
rdt.h00111 =-1/4*cumsum((b2L - 2*b3L.*etax).*betay); 
rdt.h20001 = 1/8*cumsum((b2L - 2*b3L.*etax).*betax .*exp(2i*phix)); 
rdt.h00201 =-1/8*cumsum((b2L - 2*b3L.*etax).*betay .*exp(2i*phiy)); 
rdt.h10002 = 1/2*cumsum((b2L -   b3L.*etax).*etax .* sqrt(betax) .* exp(2i*phix)); 
%
% 1st - order geometric terms
%
rdt.h21000 = -1/8*cumsum((b3L).*(betax.^(3/2)) .* exp(1i*phix)); 
rdt.h30000 = -1/24*cumsum((b3L).*(betax.^(3/2)) .* exp(3i*phix)); 
rdt.h10110 =  1/4*cumsum((b3L).*(betax.^(1/2)) .*(betay) .*exp(1i*phix)); 
rdt.h10020 =  1/8*cumsum((b3L).*(betax.^(1/2)) .*(betay) .*exp(1i*(phix-2*phiy))); 
rdt.h10200 =  1/8*cumsum((b3L).*(betax.^(1/2)) .*(betay) .*exp(1i*(phix+2*phiy))); 
