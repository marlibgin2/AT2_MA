...
function [x0pos, y0pos, nuxpos, nuypos, diffuvec, Ae] = DA_NU_FMA_par(ring,filename,nturn,nx,ny,xmax,ymax)
...
if calc==1
    if controlgcp==1
        ...
        %%%%parpool('aurora R2022a',12) %%% TEST with reduced nodes 
        parpool('local',12)
        pp = gcp; 
    end
...
% MA 05022024 - test
%
period = 1; % 1=fullring/20=achromat
for i = 1:size(r,2) %parfor
    ...
    if(isnan(xfpeak) || isnan(zfpeak) || (xfpeak == 0) || (zfpeak == 0)) % with lost particles we save zeros ...
       ...
       nux(i) = abs(freq(b)/2/pi) * period; 

       [freq amplitude phase] = calcnaff(xhalf(3,:), xhalf(4,:)); 
       [a b] = max(abs(amplitude)); 
       nf = length(freq); if (nf < nfreq), freq((nf+1):nfreq) = 0.0; end;
       nuy(i) = abs(freq(b)/2/pi) * period; 
       ...
       [a b] = max(abs(amplitude)); 
       nf = length(freq); if (nf < nfreq), freq((nf+1):nfreq) = 0.0; end;
       nux_2 = abs(freq(b)/2/pi) * period; 

       [freq amplitude phase] = calcnaff(xhalf(3,:), xhalf(4,:)); 
       [a b] = max(abs(amplitude)); 
       nf = length(freq); if (nf < nfreq), freq((nf+1):nfreq) = 0.0; end;
       nuy_2 = abs(freq(b)/2/pi) * period; 
