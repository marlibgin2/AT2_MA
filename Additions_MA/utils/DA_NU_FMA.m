...
% --------------------------------------------------
% from a Diamond code developed for AT1.3
% refurbished to use calcnaff.m from AT2.0 libraries
% MA 10/11/2023
% --------------------------------------------------

function [x0pos, y0pos, nuxpos, nuypos, diffuvec] = DA_NU_FMA(ring,filename,nturn,nx,ny,xmax,ymax)
...
if display_output
    ...
    % title(tstr1);
    title('Dynamic Aperture at STR01')
    ...
    res_order = 5;
    plot_net(res_order,qrange(1),qrange(2),qrange(3),qrange(4));
    
    tstr2 = ['Tune Footprint']; %['Frequency Map'];
    title(tstr2);
    xlabel('\nu_x');
    ylabel('\nu_y');
