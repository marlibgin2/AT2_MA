...
% try
    if verbo; disp('CON -- Calculating Twiss parameters'); end

[twiss, tune, chrom]  = twissring(rrr,0.0, 1:length(rrr), 'chrom', 1e-7);

% If tunes and chromaticities could be calculated, proceed
if all(~isnan(tune)) || all(~isnan(chrom))
    ...
    % end
    if verbo; disp('        SUCCESS. Returning results to GA solver for constraint evaluation.'); end
...
%     if verbo; disp('CON -- Tune and/or chrom. equal to NaN, indicating unstable lattice...'); end
    if verbo; disp('        FAILURE. Flagging non-fulfilled constraints to GA solver.'); end
