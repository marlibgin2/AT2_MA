function Summary = atsummary(RING)
...
% Structure to store info - remove any dependence from MML, MA 07032024
Summary.e0 = RING{1}.Energy/1e9; % getenergy('Model');
Summary.circumference = findspos(RING, length(RING)+1);
...
[TD, Summary.tunes, Summary.chromaticity] = twissring(RING, 0, 1:length(RING)+1, 'chrom', 1e-8);
Summary.compactionFactor = mcf(RING);
...
for i = 1:length(RING)
    if isfield(RING{i}, 'BendingAngle') && isfield(RING{i}, 'EntranceAngle')
        ...
        Summary.integrals(1) = Summary.integrals(1) + dispersion*RING{i}.Length/rho;
        Summary.integrals(2) = Summary.integrals(2) + RING{i}.Length/(rho^2);
        Summary.integrals(3) = Summary.integrals(3) + RING{i}.Length/(rho^3);
        % For general wedge magnets
        Summary.integrals(4) = Summary.integrals(4) + ...
            D_x(i)*tan(RING{i}.EntranceAngle)/rho^2 + ...
            (1 + 2*rho^2*RING{i}.PolynomB(2))*(D_x(i)+D_x(i+1))*RING{i}.Length/(2*rho^3) + ...
            D_x(i+1)*tan(RING{i}.ExitAngle)/rho^2;
        %         Summary.integrals(4) = Summary.integrals(4) + 2*0.5*(D_x(i)+D_x(i+1))*RING{i}.Length/rho^3;
        ...
        Summary.integrals(5) = Summary.integrals(5) + RING{i}.Length*(H1+H0)*0.5/(rho^3);
        %         if H1+H0 < 0
        %             fprintf('%f %i %s\n', H1+H0, i, RING{i}.FamName)
        %         end
        Summary.integrals(6) = Summary.integrals(6) + RING{i}.PolynomB(2)^2*dispersion^2*RING{i}.Length;
    end
end
...
if ~isempty(cavind)
    ...
    v_cav   = getcellstruct(RING,'Voltage',cavind);
    I       = findcells(RING,'TimeLag');             % TimeLag is used instead of PhaseLag in the CavityPass passmethod, but might not be defined
    if ~isempty(I)
        cT  = zeros(size(v_cav));
        [~, I, ~] = union(I,cavind);
        cT(I)  = getcellstruct(RING,'TimeLag',cavind(I));
    else
        cT  = zeros(size(v_cav));
    end
%     phi = getcellstruct(RING,'PhaseLag',cavind);
    phi     = 2*pi * cT ./ (299792458 ./ RING{cavind(1)}.Frequency);
