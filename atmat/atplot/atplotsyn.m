...
if (~isempty(xplot) && ~isempty(yplot))
    [bendType, ColCode] = findspecialdipoles(dipoles,rok); % Identify anti-bends, transv.grad bends and assign colors
    ii=[];
...
pureSext = find(atgetfieldvalues(rok(mpoles),'PolynomB',{3})~=0&atgetfieldvalues(rok(mpoles),'PolynomB',{2})==0);
pureQuad = find(atgetfieldvalues(rok(mpoles),'PolynomB',{2})~=0&atgetfieldvalues(rok(mpoles),'PolynomB',{3})==0);
mpolesS  = false(size(mpoles)); mpolesQ  = false(size(mpoles));
