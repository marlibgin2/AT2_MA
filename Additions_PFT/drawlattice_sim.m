function h = drawlattice_sim(RING, Offset, Scaling, hAxes, Ldraw)
%DRAWLATTICE - Draws the AT lattice to a figure
%  h = drawlattice(RING, Offset {0}, Scaling {1}, hAxes {gca}, Ldraw)
%
...
%  Modified by Pedro F. Tavares 2023/02/05
CorrectorFlag = 1;
...
for i = 1:length(THERING)
    ...
    elseif isfield(THERING{i},'PolynomB') && length(THERING{i}.PolynomB)>2 && (THERING{i}.PolynomB(3) || any(strcmpi(THERING{i}.FamName,{'SF','SFF','SD','SDD','HSF','HSD','SFI_F','SD_F'})))
        % Sextupole
        %Shrinkage = .05;
        NumberOfFinds = NumberOfFinds + 1;
