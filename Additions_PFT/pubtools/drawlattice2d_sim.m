function h = drawlattice2d_sim(RING, StartAngle, Offset, Scaling, hAxes, Ldraw)
%DRAWLATTICE2D - Draws the AT lattice to a figure
%  h = drawlattice2d(RING, StartAngle {0}, Offset {0}, Scaling {1}, hAxes {gca}, Ldraw)
%
...
%Modified by Pedro F. Tavares 2022/02/04
% global THERING
...
for i = 1:length(THERING)
    ...
    % mod Pedro F. Tavares 2022/02/05
     if(any(strcmpi(THERING{i}.FamName,{'dip','dipm','Kicker'})))
    ...
    elseif isfield(THERING{i},'PolynomB') && length(THERING{i}.PolynomB)>2 && (THERING{i}.PolynomB(3) || any(strcmpi(THERING{i}.FamName,{'SF','SFF','SD','SDD','HSF','HSD','SD_F','SFI_F'})))
        % Sextupole
        NumberOfFinds = NumberOfFinds + 1;
        if THERING{i}.PolynomB(3)>0 || any(strcmpi(THERING{i}.FamName,{'SF','SFF','HSF','SFI_F'}))
            % Focusing sextupole
            IconHeight = .7;
            IconColor = [1 0 1];
            IconWidth = THERING{i}.Length;
