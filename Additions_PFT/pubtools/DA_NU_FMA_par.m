...
if calc==1
    ...
    DR = da_fma_fast_mach(ring, nturn, r, [d outfile], display_output);    
end
if controlgcp==1
    delete(pp)
end
...
function DR = da_fma_fast_mach(RING,  nturn, r, outfile, display_output)
vv = [];
syms vv;
v  = {};
syms v; 
...
dnu   = sqrt(dnuxt.*dnuxt + dnuyt.*dnuyt);
dnuxt(isnan(dnuxt))=0;
dnuyt(isnan(dnuyt))=0;
...
dnu(nux==0)=nan; % lost particles
dnu(nuy==0)=nan; % lost particles

dnu(dnu==0)= 1E-10; % perfect (no diffusion points)
DRi = log10(dnu);
DRi(isnan(DRi)) = 0;
DR = sum(DRi)/(nx*ny);
