function f = LattOpt_EmitDiffRate(x, LatticeOptData)
%% input: 
...
%
%% Parameters for Diffusion rate calculation
%
...
nturnsDR   = DAoptions.nturnsDR;
nx         = DAoptions.npdax*2+1;
ny         = DAoptions.npday+1;
...
%
% Calculates Total Diffusion Rate
%
       try
           [~, ~, ~, ~, ~, DR]=DA_NU_FMA_par(ACHRO,'fma',nturnsDR,nx,ny,XmaxDA,YmaxDA,'newlcalc');
            f(2) = DR;
       catch ME
            fprintf('Error in LattOpt_EmitDiffRAte: Diffusion Rate calculation \n');
            fprintf('Error message was: %s \n',ME.message);    
       end
    else
       f(1)=Inf;
