...
if (strcmp(compf,'Y'))
   ...
   for i=1:pfsize
       try
        rpar=ExMOGA(MOGAResults,-i,'N','N', 'N', 'N', 'N', 'N', [], [],[],'N', [ ], [ ],'N','N',LatticeOptData.DAoptions);
        ...
        if (isfield(rpar,'SD'))
            MOGAResults.ParetoFront(i,nvars+nfitvars+7)=rpar.SD;
        else
            MOGAResults.ParetoFront(i,nvars+nfitvars+7)=rpar.Sc1;
        end
        if (isfield(rpar,'SFI'))
            MOGAResults.ParetoFront(i,nvars+nfitvars+8)=rpar.SFI;
        else
