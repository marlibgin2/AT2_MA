...
% FIXES
% I realized also that the units of length in the dipole slice tabs are incorrect – they are meters and not mm now.
% The slice names in the dipole slice tables are now all the same. In the original lattice files these did have different name (D0, DF1, Df2, etc), but now the usefulness of those different names is not so obvious anymore. I suggest we simply have a sequential number in that column.
% Provide a default name for saving the excel file, based on the Lattice Name field of the input structure.

% 1.       Add units to the columns that still do not have it.
% 2.
% 3.       
% 4.       Change the order of the tabs to: LatticeCoordinates_NoSlices, D1, D2, D3, LatticeCoordinates_Detailed, MagParams, Orbit, RingParameters
...
% The existing RF system is assumed to be kept
totalVoltage = 6*300e3;
harmNumber = round(circumference * 100e6/3e8);
cavFreq = harmNumber * physconst('LightSpeed')/circumference;
particleEnergy = 3e9;

% Expand to a full ring
N = ones(20,1)*(1:numel(ACHR)); N = N'; N = N(:);
RING = ACHR(N);

% Get rid of any duplicated cavities
I = setdiff(findcells(RING,'Frequency'),findcells(ACHR,'Frequency'));
RING(I) = [];

% Get rid of any duplicated RingParam elements, one is enough
...
% Determine whether the lattice contains a cavity, which is needed for some
% of the parameter calculations. If not, add a thin one. NB! Possible issue
% due to no ThinCavityPass.
cavpts = findcells(RING,'Frequency');
if isempty(cavpts)
    cavpts = numel(RING)+1;
    RING{cavpts} = atrfcavity('CAVITY',0,totalVoltage,cavFreq, harmNumber, particleEnergy);
end

% Ensure the ring properties are correctly set
RING = atSetRingProperties(RING,'cavpts',cavpts,'HarmNumber',harmNumber,'rf_voltage',totalVoltage);
