function saveOPA(XAll,Ks,Scs,LatticeOptData,RunNumber,varargin)
% Saves an OPA input file corresponding to an AT 2.0 input
% The AT 2.0 input is defined by a vector of magnet strengths for either all
% magnet families (XAll) or a set of decision variables (Ks) and two
% sextupole strengths setting chromaticity (Scs).
% The optional flag "oct" indicates XAll includes octupole families
% included.
% Use of XAll is recommended, whereas Ks and Scs are maintained for
% backward compatibility. The structure LatticeOptData (generated by running max4U.m
% defines the lattice structures to which these vectors apply.
%   
%% preamble
...
%% Input argument parsing
    octf = any(strcmpi(varargin,'Oct')); % XAll includes octupoles
...
OPA_TemplatesFolder = '/home/pedtav/Documents/Max4U/OPA_Templates/';
OPA_InputsFolder    = '/home/pedtav/Documents/Max4U/OPA_Inputs/';
switch lattMode
     case 'b1'
        switch optMode
          case 'CHRO'
     ...
     case 'b3'
         if (octf)
            filein  = 'm4U_MSOGA_Template_b3O.opa';
         else
...
%
 ...
 %
  ...
  if (isfield(LatticeOptData,'All_famsO'))
     for i=1:LatticeOptData.nallfamsO
         fprintf(fileIDout, 'DV%02d  = %8.5f ; { %s } \r\n',i,XAll(i),LatticeOptData.All_famsO{i});
     end
  end
end
