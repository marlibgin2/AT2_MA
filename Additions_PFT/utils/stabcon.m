...
%          c(11)= Jx - 3.0
%          c(12)= sqrt(sum(alpha_x^2)) - calculated at UC border markers
%          c(13)= sqrt(sum(alpha_6^2)) - calculated at UC border markers 
%          c(14)= abs(etaxP) - calcuated at UC border markers
%
...
% PFT 2024/08/31 : added alphas at unit cell borders
%                  fixed handling of nan for BetaXMAx, BetaYMax
% PFT 2024/09/01 : added etaxp at unit cell borders
...
if (isfield(LatticeOptData,'AlphaUCb'))
    AlphaUCb    = LatticeOptData.AlphaUCb;
else
    AlphaUCb    = NaN;
end

if (isfield(LatticeOptData,'EtaxPUCb'))
    EtaxPUCb    = LatticeOptData.EtaxPUCb;
else
    EtaxPUCb    = NaN; 
end

if (isfield(LatticeOptData,'IfamsUCborderF'))
    IUCbs = LatticeOptData.IfamsUCborderF;
else
    IUCbs       = [];
end
...
if ( (c(1)<0)&&(c(2)<0) )
    ...
    if (isnan(AlphaUCb))
        ...
        alphas  = cat(1,TD.alpha);
        AlphaXs = alphas(IUCbs,1);
        AlphaYs = alphas(IUCbs,2);
        c(12) = sqrt(sum(AlphaXs.*AlphaXs))-AlphaUCb;
        c(13) = sqrt(sum(AlphaYs.*AlphaYs))-AlphaUCb;
    end
    
    if (isnan(EtaxPUCb))
        c(14)=-1;
    else
        etaxs  = cat(3,TD.Dispersion);
        etaxps = etaxs(IUCbs);
        c(14)  = sqrt(sum(etaxps.*etaxps));
    end
